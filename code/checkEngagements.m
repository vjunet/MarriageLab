function [ stable,counter ] = checkEngagements2( engaged, m, f )
...
%   returns true for stable engagements, false otherwise; return "counter",
%   the number of unstable mariages

n = size(m,1);

%invert the engaged matrix such that the neu matrix has the index of the
%women on the column one and those of their respective husband in row two
TransitionMatrix=zeros(n,2);
invengaged=zeros(n,2);
TransitionMatrix(:,1)=engaged(:,2);
TransitionMatrix(:,2)=engaged(:,1);
i=1;
while i~=n+1
    index=TransitionMatrix(i,1);
    while index==0 && i~=n
        i=i+1;
        index=TransitionMatrix(i,1);
    end %while
    ...
    invengaged(index,:)=TransitionMatrix(i,:);
    i=i+1;
end %while
invengaged;
stable=true;
...
while he<=n
    she = engaged(he,2); %"she" is engaged to "he"
        ...
        while she==0 && he~=n   %"he" is not engaged, so there isn't unstability ==> check the next man
            he = he+1;
            she = engaged(he,2);
        end %while
